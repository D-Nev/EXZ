...
public:
    struct User {
        ...
        string phoneNum;
        string login;
        string password; //зашифрованном виде
...
private:
    ...
    void viewAllUsers() {
        ...
        for (const auto& user : users) {
            cout << "Логин: " << user.login << ", Имя: " << user.name << "\n";
        }
    }
    ...
    void viewStats() {
        ...
        cout << "Статистика пользователей:\n";
        double totalPercentage = 0;
        int totalTests = 0;

        for (const auto& user : userResults) {
            const string& login = user.first;
            const vector<int>& results = user.second;
            ...
            double avgPercentage = static_cast<double>(sum) / results.size();
            cout << "Логин: " << login
                << " | Пройдено тестов: " << results.size()
                << " | Средний процент: " << avgPercentage << "%\n";
        ...
        if (totalTests > 0) {
            cout << "Общий средний процент по системе: "
                << static_cast<double>(totalPercentage) / totalTests << "%\n";
        }
    }
};
